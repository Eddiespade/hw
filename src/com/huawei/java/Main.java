...
public class Main {
    ...
    public static int qosLimit;
    public static int percentFive;

    public static void main(String[] args) throws IOException {
        // 加载文件
        LoadFiles.load();
        // 获取5%的点
        percentFive = (ReadDemandFile.getNeedCount() * 5) / 100 * siteMap.keySet().size();
        // 读取当前需求，每次只读取一行
        // curDemand 主要存储两个信息：当前时刻 及 每个客户所需要的带宽
        Demand curDemand = null;
        while ((curDemand = ReadDemandFile.read()) != null) {
            // 对边缘节点的带宽重置
            init();
//             String[] curAssignment;
//             // // 对当前的需求进行分配
//             if (percentFive > 0) curAssignment = dispatchBandWidth(curDemand);
//             else curAssignment = averagePlan(curDemand);
//             // 写入到输出文件中
            String[] curAssignment = averagePlan(curDemand);
            WriteOutputFile.write(curAssignment);
        }
    ...
    // 利用网络流进行分配
    public static String[] dispatchBandWidth(Demand curDemand) {
        // 创建分配方案
        // demandsMap <customID, needBandWidth>;
        Map<String, Integer> demandsMap = curDemand.getCustomDemands();
        PriorityQueue<String> custom = new PriorityQueue<>((o, p) ->
                // 按可分配节点的个数从小到大分配
                customMap.get(o).getSiteCount() - customMap.get(p).getSiteCount()
        );
        custom.addAll(demandsMap.keySet());
        String[] ans = new String[demandsMap.size()];

        /* 思路：
                1. 每次分配一个客户，用一个边缘节点去尽量满足它
                2. 分配的时候记录使用的哪个边缘节点
                3. 分配下一个节点的时候优先使用之前用过的节点去分配
                4. 到下一个时刻分配的时候，用过的所有5%都要减一
        */
        int index = 0;
        while (!custom.isEmpty()) {
            ...
            // 获取当前满足当前客户qos的边缘节点
            PriorityQueue<SatisfySite> satisfySite = curCustom.getCopyOFSatisfy();
            // 获取当前客户的需求
            int needBandWidth = demandsMap.get(curCustomID);
            // 开始分配
            while (needBandWidth > 0 && !satisfySite.isEmpty()) {
                SatisfySite cur = satisfySite.poll();
                String curID = cur.getSiteID();
                int curRes = siteMap.get(curID).getResidualBandWidth();
                if (curRes >= needBandWidth) {
                    siteMap.get(curID).setResidualBandWidth(curRes - needBandWidth);
                    updateString(curCustomAssignment, curID, needBandWidth);
                    satisfySite.offer(cur);
                    needBandWidth = 0;
                } else {
                    siteMap.get(curID).setResidualBandWidth(0);
                    updateString(curCustomAssignment, curID, curRes);
                    needBandWidth -= curRes;
                }
                // 标记已使用
                siteMap.get(curID).setBeUsed(1);
            }
    ...
    // 平均分配方案
    public static String[] averagePlan(Demand curDemand) {
        // 创建分配方案
        ...
        while (!custom.isEmpty()) {
            ...
            Custom curCustom = customMap.get(curCustomID);
            // 获取当前满足当前客户qos的边缘节点
            PriorityQueue<SatisfySite> satisfySite = curCustom.getSatisfySiteList();
            // 获取满足的个数
            int siteCount = curCustom.getSiteCount();
            // 获取当前客户的需求
            int needBandWidth = customDemands.get(curCustomID);
            // 平均分配的话，每个边缘节点需要的带宽
            int average = needBandWidth / siteCount + 1;
            // 开始分配
            while (needBandWidth > 0 && satisfySite.peek() != null) {
                ...
                } else if (residualBandWidth > needBandWidth) {
                    // 更新方案
    ...
    // 将边缘节点的剩余带宽全部置为其总带宽, 是否使用设为0
    public static void init() {
        for (String s : siteMap.keySet()) {
            Site curSite = siteMap.get(s);
            if (curSite.getBeUsed() == 1) {
                percentFive--;
                curSite.setPercentFiveCNT(curSite.getPercentFiveCNT() - 1);
                curSite.setBeUsed(0);
            }
